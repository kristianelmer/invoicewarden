...
export async function POST(request: Request) {
  ...
  const session = await stripe.checkout.sessions.create({
    mode: "payment",
    success_url: `${baseUrl}/dashboard?payment=success&invoice=${invoice.id}`,
    cancel_url: `${baseUrl}/dashboard?payment=cancelled&invoice=${invoice.id}`,
    line_items: [
      {
        quantity: 1,
        price_data: {
          ...
          product_data: {
            name: `Invoice ${invoice.invoice_number} (updated statutory total)`,
            description: `Includes statutory adjustments under ${invoice.jurisdiction}`,
          },
        },
      },
    ],
